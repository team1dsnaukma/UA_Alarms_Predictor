...
def forecast_all_regions(token, data_folder, utc_time):
    regions = pd.read_csv(data_folder + '/regions.csv')
    pd_list = []
    for name in regions["center_city_ua"]:
        if name == "Київ":
            pd_list.append(get_weather_next_12_hours("Kyiv", utc_time, token))
        elif name == "Суми":
            pd_list.append(get_weather_next_12_hours("Суми, Україна", utc_time, token))
        else:
            pd_list.append(get_weather_next_12_hours(name, utc_time, token))
...
if __name__ == "__main__":
    ...
    forecast_all_regions(TOKEN, DATA_FOLDER, utc_now)
